...
st.set_page_config(page_title='Modelo Preditivo - CPC', layout='wide')

# Título do aplicativo
st.title('Modelo Preditivo - CPC')
...
# Avaliar o modelo
mse_cpc = mean_squared_error(y_test, y_pred_cpc)
r2_cpc = r2_score(y_test, y_pred_cpc)

# Exibir métricas do modelo
st.subheader('Avaliação do Modelo')
st.write(f"Mean Squared Error: {mse_cpc:.4f}")
st.write(f"R^2 Score: {r2_cpc:.4f}")
...
# Fazer a previsão
cpc_previsto = model_cpc.predict(input_data_scaled)

# Exibir o resultado da previsão
st.subheader('Resultado da Previsão')
st.write(f"O CPC previsto é: R$ {cpc_previsto[0]:.2f}")